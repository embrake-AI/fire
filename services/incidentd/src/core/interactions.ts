...
export async function startIncident({
    ...
    const result = await incident.start({ id: incidentId.toString(), prompt, createdBy, source })
    await c.env.incidents.prepare('INSERT INTO incident (id, identifier, status, assignee, severity) VALUES (?, ?, ?, ?, ?)').bind(incidentId.toString(), identifier, 'open', result.assignee, result.severity).run()
    return result
}

export async function listIncidents({ c }: { c: EnvContext }) {
    const incidents = await c.env.incidents.prepare('SELECT id, identifier, status, assignee, severity, createdAt FROM incident').all<{ id: number, identifier: string, status: string, assignee: string, severity: string, createdAt: string }>()
