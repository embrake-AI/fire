...
describe("Incident DO core contracts", () => {
	...
	it("stores SIMILAR_INCIDENT as unpublished outbox event and deduplicates by run+incident", async () => {
		...
		const first = await stub.recordAgentInsightEvent({
			...
			eventData: {
				...
				title: "Previous API 503 incident",
				incidentStatus: "resolved",
				summary: "Very similar symptom pattern.",
				similarities: "Both incidents show API 503 spikes after deploy.",
				learnings: "shared edge routing regression pattern",
			},
	...
	it("stores generic agent context and insight events with dedupe key semantics", async () => {
		...
		const insightEvent = {
			...
			title: "Previous outage incident",
			incidentStatus: "resolved",
			summary: "Matched outage pattern.",
			similarities: "Shared trigger and same rollback mitigation path.",
			learnings: "same deployment edge case",
		};
	...
	it("queues an agent turn from SIMILAR_INCIDENT updates (debounced)", async () => {
		...
		await stub.recordAgentInsightEvent({
			...
			eventData: {
				...
				title: "Previous API error incident",
				incidentStatus: "resolved",
				summary: "Comparable elevated API error spikes.",
				similarities: "Same error class, similar timeline around deploy.",
				learnings: "similar mitigation path expected",
			},
