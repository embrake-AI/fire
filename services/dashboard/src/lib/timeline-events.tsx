...
export const eventRegistry: EventConfigMap = {
	...
	SIMILAR_INCIDENT: {
		...
		render: ({ data }) => {
			const sources = data.sourceIncidentIds.length ? data.sourceIncidentIds.join(", ") : "none";
			return (
				<div class="space-y-2">
					<p class="text-sm text-muted-foreground">
						Matched incident <span class="font-mono">{data.similarIncidentId}</span> (sources: {sources})
					</p>
					<p class="text-sm text-muted-foreground">
